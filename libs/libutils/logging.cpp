...
void CLog::VPrintf(int level, const char *Format, va_list marker)
{
    ...
    if (m_bTimeLog) {
        ...
        SYSTEMTIME t;
        GetLocalTime(&t);
        snprintf(Buffer, sizeof(Buffer), "%02u/%02u %02u:%02u:%02u.%03lu [%03d] ", t.wDay, t.wMonth,
                 t.wHour, t.wMinute, t.wSecond, t.wMilliseconds, level);
#else
        long Time = time(NULL);
...
void CLog::PrintBufferEx(int level, char *Preffix, const unsigned char *Buffer, int BufferSize)
{
    ...
    for (i = 0; i < BufferSize; i++) {
        HexBufferPtr += snprintf(HexBufferPtr, sizeof(HexBuffer) - (HexBufferPtr - HexBuffer), "%02x ",
                                 Buffer[i]);
